...
# JavaScript invisibile per forzare lo scroll in cima alla pagina al ricaricamento
components.html(
    """
    <script>
...
# ==========================================
# 3. LOGICA DI CALCOLO PUNTEGGI (RETTIFICATA)
# ==========================================
def calcola_punteggio_pronostico(p_casa, p_trasferta, res_casa, res_trasferta, marcatori_p, marcatori_r):
    ...
    # 1. CONTROLLO RISULTATO ESATTO (Esclusivamente dai totali di squadra)
    ...
    # 3. BONUS MARCATORI ED AUTOGOL (+5 PUNTI CIASCUNO)
    if marcatori_p and marcatori_r:
        lista_p = [m.strip().lower() for m in marcatori_p.split(",") if m.strip()]
        lista_r = [m.strip().lower() for m in marcatori_r.split(",") if m.strip()]
...
# ==========================================
# 7. SEZIONE: INSERIMENTO PRONOSTICI (PERSISTENZA CORRETTA)
# ==========================================
    ...
    if not st.session_state.user:
        ...
        if not partite_aperte:
            ...
            # CARICAMENTO PERSISTENTE DAL DB
            ...
            # Parsing marcatori salvati nel DB
            val_marcatori_list = [m.strip() for m in existing_p[2].split(",")] if (existing_p and existing_p[2]) else []

            st.markdown(f"### Match: **{casa}** vs **{trasferta}**")
            
            col1, col2 = st.columns(2)
            with col1:
                g_casa = st.number_input(f"Gol Totali {casa}", min_value=0, max_value=20, value=val_casa, key=f"gc_{id_partita}")
            with col2:
                g_trasferta = st.number_input(f"Gol Totali {trasferta}", min_value=0, max_value=20, value=val_trasferta, key=f"gt_{id_partita}")

            st.divider()
            st.subheader("⚽ Dettaglio Marcatori / Autogol (Solo per Bonus)")
            
            rosa_db = run_query("SELECT nome_giocatore FROM rosa WHERE attivo = 1 ORDER BY nome_giocatore ASC", fetch="all")
            lista_rosa = [r[0] for r in rosa_db] if rosa_db else []
            
            # Aggiunta opzioni Autogol alle scelte
            lista_opzioni_marcatori = ["Autogol a Favore", "Autogol a Sfavore"] + lista_rosa
            
            # Isolamento dei nomi unici per il multiselect di default
            default_selezionati = list(set([m for m in val_marcatori_list if m in lista_opzioni_marcatori]))

            marcatori_sel = st.multiselect(
                "Seleziona i marcatori o eventuali autogol (opzionale)",
                options=lista_opzioni_marcatori,
            ...
            marcatori_finali = []
            if marcatori_sel:
                st.write("Specifica la quantita per ciascun elemento selezionato:")
                for m in marcatori_sel:
                    # Conta le occorrenze salvate precedentemente per la precompilazione
                    cnt_default = val_marcatori_list.count(m) if m in val_marcatori_list else 1
                    cnt = st.number_input(f"Quantità per: {m}", min_value=1, max_value=10, value=cnt_default, key=f"cnt_{m}_{id_partita}")
            ...
            if st.button("💾 Invia / Aggiorna Pronostico", use_container_width=True):
                ...
                limite_naive = datetime.datetime.strptime(limite_str, "%Y-%m-%d %H:%M:%S")
                limite_dt = limite_naive.replace(tzinfo=tz_roma)
            ...
            for s in schedine:
                ...
                with st.expander(f"{casa} vs {tras} — Stato: {stato.upper()} (Punti Totali: {pts_tot})"):
                    ...
                    with c1:
                        ...
                        st.write(f"Marcatori/Autogol: {m_p if m_p else 'Nessuno'}")
                    with c2:
...
# ==========================================
    ...
    if marcatori_all:
        ...
        for row in marcatori_all:
            lista = [m.strip() for m in row[0].split(",") if m.strip()]
            for m in lista:
                conteggio_m[m] = conteggio_m.get(m, 0) + 1
        
        df_m = pd.DataFrame(list(conteggio_m.items()), columns=["Giocatore/Evento", "Volte Pronosticato"]).sort_values(by="Volte Pronosticato", ascending=False)
        st.bar_chart(df_m.set_index("Giocatore/Evento"))
    else:
...
# ==========================================
    ...
    * **+5 Punti**: Assegnati per ciascun marcatore o autogol esatto azzeccato nell'elenco inviato. Se un giocatore viene pronosticato con una doppietta ed effettivamente segna 2 gol nel match omologato, verranno assegnati 10 punti bonus totali.
    """)
...
# ==========================================
    ...
    with tab_omologa:
        ...
        if not partite_da_omologare:
            ...
            st.divider()
            st.subheader("Marcatori / Autogol Reali Incontro")
            rosa_db = run_query("SELECT nome_giocatore FROM rosa WHERE attivo = 1 ORDER BY nome_giocatore ASC", fetch="all")
            lista_rosa = [r[0] for r in rosa_db] if rosa_db else []
            lista_opzioni_omologa = ["Autogol a Favore", "Autogol a Sfavore"] + lista_rosa

            m_reali_sel = st.multiselect("Seleziona marcatori ed autogol registrati", options=lista_opzioni_omologa, key="om_m_select")
            
            m_reali_finali = []
            if m_reali_sel:
                for mr in m_reali_sel:
                    cnt_m = st.number_input(f"Numero occorrenze per {mr}", min_value=1, max_value=10, value=1, key=f"om_cnt_{mr}")
    ...
    with tab_rosa:
        ...
        if st.button("➕ Aggiungi Giocatore"):
            if new_player:
                run_query("INSERT INTO rosa (nome_giocatore, ruolo, attivo) VALUES (?, ?, 1)", (new_player, role_player))
                log_admin_action(st.session_state.user, f"Aggiunto giocatore {new_player}")
                st.success(f"Giocatore {new_player} aggiunto correttamente!")
                st.rerun()
    ...
    with tab_utenti:
        ...
        if st.button("👤 Crea Account"):
            if u_name and u_pass:
                try:
                    now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    run_query("INSERT INTO utenti (username, password, ruolo, data_iscrizione) VALUES (?, ?, ?, ?)", (u_name, u_pass, u_role, now_str))
                    log_admin_action(st.session_state.user, f"Registrato nuovo utente {u_name}")
                    st.success(f"Utente {u_name} registrato con successo!")
...
def mostra_footer():
    st.markdown("""
        <div class="cugurra-footer">
            Premio Cugurra v2.5 &copy; 2026 — Schedine & Statistiche Web App | Powered by Streamlit & SQLite
        </div>
    """, unsafe_allow_html=True)
